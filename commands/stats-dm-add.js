...
export default {
    name: "stats-dm-add",
    description: "Get validator stats via DM after each epoch",
    options: [
        {
            name: 'name',
    ...
    run: async (client, interaction, args) => {
       ...
       const validatorVoteAddress = interaction.options.getString('name');
        ...
        const text = new TextDisplayBuilder()
            .setContent(`### <:tick:1399117596749598801> Added [${info.name}](https://solscan.io/account/${info.validatorId}) to DM subscriptions.`);
        ...
        if (existingSubscription) {
            if (existingSubscription.subscribers.some(sub => sub.userId === interaction.user.id)) {
                return interaction.reply({ content: `**You are already subscribed to this validator.**`, ephemeral: true });
            }

            existingSubscription.subscribers.push({
            ...
            const newSubscription = new validatorDMSubscriptionSchema({
